...
if ($value == "entity")
{
    ...
    $monitors = $uptime_api->getMonitorStatus($monitor_id);
    $k = "elementId";
	$v = $monitors['elementId'];
	$json[$k] = $v;
...
elseif ($value == "status"){
	// Create API object
    $uptime_api = new uptimeApi($uptime_api_username, $uptime_api_password, $uptime_api_hostname, $uptime_api_port, $uptime_api_version, $uptime_api_ssl);
    $status = $uptime_api->getElementStatus($entity_id);
    if(strpos($status['status'], "MAINT") === 0)
	{
			$k = "Maintenance";
			$v = "hostOutOfMaintenance";
			$json[$k] = $v;
	}
	else 
	{
...
// Unsupported request
    ...
    echo "Error: Unsupported Request '$value'" . "</br>";
    }
